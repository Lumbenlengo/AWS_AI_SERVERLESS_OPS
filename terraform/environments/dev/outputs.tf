...
output "demo_app_ecr_url" {
  ...
  value       = try(module.ecs_app[0].ecr_repository_url, null)
}

output "demo_app_cluster" {
  value = try(module.ecs_app[0].cluster_name, null)
}

output "demo_app_service" {
  value = try(module.ecs_app[0].service_name, null)
}

output "demo_app_watch_alarm" {
  description = "Confirm this name matches what REMEDIATION_MAP expects"
  value       = try(module.ecs_app[0].watch_alarm_name, null)
}